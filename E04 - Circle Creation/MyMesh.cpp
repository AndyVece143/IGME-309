...
void MyMesh::GenerateCircle(float a_fRadius, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	*/
	...
	{
		vector3 coolVector = vector3(cos(theta) * a_fRadius, sin(theta) * a_fRadius, 0.0f);
		theta += delta;
	...
	for (int i = 0; i < a_nSubdivisions; i++)
	{
		AddTri(vector3(0, 0, 0), vertex[i], vertex[(i + 1) % a_nSubdivisions]);
	}
	...
	AddTri(vector3(1.0f, 1.0f, 0.0f),
		vector3(2.0f, 1.0f, 0.0f),
		vector3(1.77f, 1.77f, 0.0f));
		*/
